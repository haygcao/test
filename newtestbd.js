...
handleResponse(jsonData) {
  try {
    ...
    if (response.body && response.body.includes('<html')) {
      // 自然加载网页内容
      this.loadHtmlContent(response.body, requestId);
    }
  } catch (error) {
...
handleInterceptedData(jsonData) {
  try {
    ...
    if (action === 'processHtml' && payload.html) {
      // 自然加载网页内容
      this.loadHtmlContent(payload.html, payload.requestId);
    }
  } catch (error) {
...
extractDataFromDOM(doc) {
  try {
    ...
    // 尝试从标准元素中提取数据
    const phoneElement = doc.querySelector('.tel-num');
    const locationElement = doc.querySelector('.location');
    const tagElement = doc.querySelector('.report-name');
    const typeElement = doc.querySelector('.report-type');
    ...
    if (tagElement) {
      ...
      for (const key in manualMapping) {
        if (result.tag.includes(key)) {
          result.predefinedLabel = manualMapping[key];
          break;
        }
      }
      
      if (!result.predefinedLabel) {
 ...
 * @param {string} requestId - 请求ID
 */
loadHtmlContent(html, requestId) {
  try {
    // 创建一个临时的DOM解析器来提取数据
    ...
    // 自然加载HTML内容
    this.processAndLoadHtml(html, extractedData, requestId);
  } catch (error) {
    this.logError('Error loading HTML content:', error);
    this.sendPluginError('Failed to load HTML content: ' + error.message, requestId);
  }
}
 ...
 * @param {string} requestId - 请求ID
 */
processAndLoadHtml(html, extractedData, requestId) {
  try {
    // 创建一个新的文档
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');
    ...
    this.logError('Error processing and loading HTML:', error);
    this.sendPluginError('Failed to process and load HTML: ' + error.message, requestId);
  }
}
