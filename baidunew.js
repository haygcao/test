(function () {
    ...
    function replaceEntireDocument(htmlString) {
        try {
            console.log('Starting safe document replacement...');
            console.log('Original HTML length:', htmlString.length);
            
            // 使用DOMParser安全解析HTML，避免CORS和编码问题
            ...
            if (newDoc.documentElement) {
                ...
                if (newHead && currentHead) {
                    // 保存当前页面的关键脚本（避免丢失插件功能）
                    const criticalScripts = [];
                    const currentScripts = currentHead.querySelectorAll('script');
                    currentScripts.forEach(script => {
                        if (script.src && (script.src.includes('resource_interceptor') || 
                            script.textContent.includes('BaiduPhoneSearchPlugin'))) {
                            criticalScripts.push(script.cloneNode(true));
                        }
                    });
                    ...
                    // 复制新head的所有内容
                    const newHeadChildren = Array.from(newHead.children);
                    newHeadChildren.forEach(child => {
                        const clonedChild = child.cloneNode(true);
                        
                        // 对于script标签，需要重新创建以确保执行
                        if (clonedChild.tagName === 'SCRIPT') {
                            ...
                            // 复制内容
                            if (clonedChild.src) {
                                newScript.src = clonedChild.src;
                            } else {
                                newScript.textContent = clonedChild.textContent;
                            }
                            
                            currentHead.appendChild(newScript);
                        } else {
                            currentHead.appendChild(clonedChild);
                        }
                    });
                ...
                if (newBody && currentBody) {
                    ...
                    // 复制新body的所有内容
                    const newBodyChildren = Array.from(newBody.children);
            ...
            console.error('Error in safe document replacement:', error);
            
            // 备用方案：尝试提取并正确设置head和body
            try {
                console.log('Attempting fallback head/body extraction...');
