...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === `xhrResponse_${pluginId}`) {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      // 将数据传递回 Flutter, 现在什么都不做, 因为只是测试能不能收到数据
      FlutterChannel.postMessage(JSON.stringify({
        type: 'pluginResult',
        pluginId: pluginId,
        data: {},
      ...
      // resolve 对应的 Promise, 现在什么都不做, 因为只是测试能不能收到数据
  ...
  } else if (event.data.type === 'testEvent') {
      console.log('Received testEvent:', event.data.detail);
  } else {
